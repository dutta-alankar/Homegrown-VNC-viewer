...
void VncClientSession::runWorker(QString host, int port, QString password) {
    ...
    while (m_running && client) {
        const int waitRes = WaitForMessage(client, 500);
        if (waitRes < 0) {
            break;
        }
        if (waitRes > 0) {
...
void VncClientSession::emitFrame() {
    ...
    const QImage image(reinterpret_cast<const uchar*>(m_client->frameBuffer), width, height, stride, QImage::Format_RGB32);
    emit frameUpdated(image.copy());
}
